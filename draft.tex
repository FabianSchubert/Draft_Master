...
Theoretical models of homeostasis are generally based on a form of feedback control. As such, it necessarily contains an error estimation. This could be based e.g. on the neuron's firing rate of the recent past measured in a given window, or, on a physiological level, e.g. a concentration of some molecule within some part of the cell structure. In any case, the error estimation requires setting some predefined target that the feedback control will attempt to achieve. This raises two questions: First, what would be an appropriate choice of homeostatic targets? Second, how could a neural system with fixed homeostatic targets encode changes of external stimuli if homeostasis overrides any information about changes in synaptic input?
