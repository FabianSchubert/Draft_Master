...
In this section, we discuss an analytic treatment of the dynamics underlying the diffusive homeostatic mechanism. The goal of this analysis is to predict the shape of the power spectrum shown in Figure \ref{Power_Spec_without_Analysis}. This will give insight into the relation between parameters of the model and the resulting preferred frequency and amplitude, allowing us to choose appropriate parameter values in order to reduce frequency and amplitude of the oscillations.
...
\begin{align}
\begin{split}
\dot{NO}(t) &= - \lambda NO + \frac{ N_{exc.} {Ca^{2+}_{spike}}^3 \tau_{Ca^{2+}}ln(2)}{3 L^2}[\alpha(V_{t,pop.}(t)-V_{t,0}) + r_{IP}] \\
&+ \frac{{Ca^{2+}_{spike}}^3 {\tau_{Ca^{2+}}} ln(2)}{3L^2}\sqrt{\frac{N_{exc.}}{2 \tau_{nNOS}}}\sqrt{\alpha(V_{t,pop.}(t)-V_{t,0}) + r_{IP}}\xi (t)
\end{split}\label{2d-sys-homeostasis1}\\
\dot{V_{t,pop.}}(t) &= \frac{NO-NO_0}{NO_0 \tau_{V_t}}\;. \label{2d-sys-homeostasis2}
\end{align}
Through the coordinate transformation $n = NO-NO_0 \; , \theta = V_{t,pop.}(t)-V_{t,0}$, we can simplify the dynamical system:
\begin{align}
\begin{split}
\dot{n} &= -\lambda n + \frac{\alpha N_{exc.} {Ca^{2+}_{spike}}^3 \tau_{Ca^{2+}}ln(2)}{3 L^2}\theta \\
&+ \frac{{Ca^{2+}_{spike}}^3 {\tau_{Ca^{2+}}} ln(2)}{3L^2}\sqrt{\frac{N_{exc.}}{2 \tau_{nNOS}}}\sqrt{\alpha\theta + r_{IP}}\xi (t)
\end{split}\label{2d-sys-homeostasis-coord_transf1}\\
\dot{\theta} &= \frac{1}{NO_0 \tau_{V_t}} n \label{2d-sys-homeostasis-coord_transf2}
\end{align}
The amplitude of noise in equation \ref{2d-sys-homeostasis-coord_transf1} depends on $\theta$. If kept in the equation, this dependence turned out be an insuperable obstacle for our goal of finding an analytic description of the homeostatic oscillations. In consequence, we decided to make one further simplification, replacing $\sqrt{\alpha\theta + r_{IP}}$ by a constant factor $\sqrt{r_{IP}}$. 
