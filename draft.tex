...
As an additional test, we applied external poisson input for $\mathrm{t\geq 300\, s}$ to two excitatory subgroups to see whether changes in the firing rate of these subgroups is levelled out by homeostasis or retained. Members of these subgroups were picked randomly and independent of their position.
\begin{figure}
\begin{center}
%% plot_external_input_switch_activity.py
\includegraphics[width=0.7\textwidth]{../../plots/external_input_switch.png}
\end{center}
...
\section{Discussion}
By implementing diffusive homeostasis into the LIF-SORN model we managed to achieve a broader, skewed firing rate distribution while preserving appreciated features of network topology. As an additional effect, we observed the emergence of a small number of highly influential neurons, which is a feature observed in other theoretical and experimental studies. Section \ref{theor_osc} discussed the emergence of noise-induced oscillations. Suppressing this instability required homeostasis to act on a slow timescale of minutes to hours, which, however, is in accordance with experimentally observed time scales of activity modulation through nitric oxide \cite{Steinert_NO}.

In general, diffusive homeostasis is regarded as a possibility of maintaining an overall level of activity, while allowing single neurons greater variability compared to single-neuron homeostasis, be it a single or individual target rates. According to Sweeney et al., this allows for the encoding of heterogeneous input by differences in the resulting excitatory activity. For this effect to be persistent, one should be able to alter the homeostatic fixed point by changes in external input.
Our results did not support this. As shown in Figure \ref{Ext_Input_Switch}, we found that diffusive homeostasis does not maintain separability of external input by the resulting firing rates. Rather, by analysing network activity and comparing it to our analytic model, we found evidence that the fixed point of network activity is strongly determined and can be well predicted by the spatial configuration of excitatory neurons. However, the validity of this prediction declined for larger diffusion constants and completely vanished for instant diffusion. In a strict sense, only instant diffusion allows for an infinite number of activity configurations that fulfil the homeostatic condition. Presumably however, if diffusion in a real biological system happens at a finite but fast speed compared to the adaptation of intrinsic excitability, one could arguably expect it to be equivalent to instant diffusion. In our simulation, we explained the fact that we saw a steady decline of correlation in Figure \ref{Corr_Coeff_vs_D} by the decreasing capability of the homeostatic mechanism to quickly enough react to structural synaptic changes that alter the recurrent excitatory inputs and, in consequence, excitatory firing rates.

As an indirect consequence of spatial structure determining activity, we found that neurons with a low-density neighbourhood have a higher chance of developing strong outgoing weights. Of course, this raises the question, whether one can expect such an effect to be observable in the nervous system. To our knowledge, no experimental study exists that directly relates fluctuations of neuronal densities to the existence of highly influential excitatory neurons. Ivenshitz and Segal studied the influence of neuronal density onto neuronal activity in a culture of hippocampal neurons \cite{Ivenshitz_2010}. Lower densities resulted in a more irregular activity, with stronger but less frequent bursting events. However, since our network was tuned to exhibit non-bursting activity, it is hard to draw any conclusions from this result.

A major problem of the implemented model is the usage of quasi-point sources of nitric oxide, since it leads to a clear single homeostatic fixed point. Future modifications of the homeostatic model might integrate a wider-spread kernel of nNOS, allowing for a larger amount of states of activity fulfilling the homeostatic target. In fact, experimental and theoretical studies have suggested that NO is synthesized across wider spread cell structures, leading to a more even distribution of NO \cite{Philippides_2005}.

While the implemented mathematical description of diffusion (Equation \eqref{NO_dyn}) is physically based, one can question the validity of the implementation of threshold adaption (Equation \eqref{Theta_dyn}): In principle, it does not impose any bounds onto the threshold value, which, from a biological point of view, seems implausible.  
