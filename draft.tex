...
where $\tau_e$ and $\tau_i$ represents time constants representing the rapidity of firing rate adaptation, $\phi_e$ and $\phi_i$ the respective transition functions between synaptic input and firing rate and $W_{xy}$ the mean synaptic weight from population x to population y. 

As plot A* in Figure \ref{full_sim_osci} indicates, the mean firing rates of both populations are highly correlated in time. For the source of oscillating behaviour to be found within the interaction between both populations,  
