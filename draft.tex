...
\caption{Linear fit of excitatory population activity and mean excitatory threshold. Slope $\alpha$ of the fit: $\mathrm{-0.789\, Hz/mV}$. Offset $\beta$: $\mathrm{-41.97\, Hz}$. Mean squared error: $\mathrm{R^2 = 0.68}$. Colors encode the time derivative of $<V_t(t)>$, which indicates an ellipsoid like trajectory in phase space, with the firing rate following the threshold with a short delay.}
...
nos &\equiv nNOS_{total} - nNOS_{total,0} \label{simplif_coord_transf3}
\end{align}
we can simplify the dynamical system:
\begin{align}
\dot{n} &= -\lambda n + \frac{nos}{L^2} + \sigma_{NO} \xi(t) \label{2d-sys-homeostasis-coord_transf1}\\
\dot{\theta} &= \frac{1}{NO_0 \tau_{V_t}} n \label{2d-sys-homeostasis-coord_transf2} \\
\dot{nos} &= \frac{\alpha \cdot N_{exc} \cdot \gamma \cdot \theta - nos }{\tau_{nNOS}} \label{2d-sys-homeostasis-coord_transf3}
\end{align}
...
nNOS_{total,0} &= r_{IP}\cdot N_{exc.} \cdot \gamma \; . \label{2d-sys-fixed-points2}
\end{align}
As stated in the beginning, we were interested in finding an analytic expression for the power spectrum of the mean excitatory threshold. Therefore we took the fourier transform of \eqref{2d-sys-homeostasis-coord_transf1} - \eqref{2d-sys-homeostasis-coord_transf3}, yielding
\begin{align}
i\omega f_n &= - \lambda f_n + \frac{f_{nos}}{L^2} + \sigma_{NO} f_{\xi} \label{2d-sys-homeostasis-coord_transf_ft1}\\
i\omega f_\theta &= \frac{f_n}{NO_0\tau_{V_t}} \label{2d-sys-homeostasis-coord_transf_ft2} \\
i\omega f_{nos} &= \frac{\alpha \cdot N_{exc.} \cdot \gamma f_\theta - f_{nos}}{\tau_{nNOS}} \label{2d-sys-homeostasis-coord_transf_ft3}
\end{align}
...
\begin{equation}
P_\theta (\omega) = \frac{\sigma_{NO}^2}{\omega^2 \left( \lambda NO_0 \tau_{V_t} + \frac{\alpha N_{exc.} \gamma \tau_{nNOS}}{L^2(\omega^2\tau_{nNOS}^2 + 1)} \right)^2 + \left( \omega^2 NO_0 \tau_{V_t} + \frac{\alpha N_{exc.} \gamma}{L^2 (\omega^2\tau_{nNOS}^2 + 1)}\right)^2} \label{Pow_Spec_Theta_Final}
\end{equation}
...
Figure \ref{Pow_Spec_Theta_vs_Analytic} shows the result of \eqref{Pow_Spec_Theta_Final}. 
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{../../plots/power_spec/power_spectrum_analytic.png}
\end{center}
\caption{Power Spectrum of $\mathrm{\langle V_t \rangle}$ (blue) and analytic predictions based on Equation \eqref{Pow_Spec_Theta_Final}. Red curve was fitted by tweaking only the overall amplitude $\sigma_{NO}$ \eqref{Pow_Spec_Theta_Final} ($\mathrm{\alpha = -0.789 \,Hz/mV}$, as in Figure \ref{thresh_r_linfit}). Green curve is a fit achieved by adjusting $\sigma_{NO}$ and $\alpha$ in \eqref{Pow_Spec_Theta_Final} ($\mathrm{\alpha = -1.114 \,Hz/mV}$).}
\label{Pow_Spec_Theta_vs_Analytic}
\end{figure}
Apparently, the value for $\alpha$ that was determined by means of the linear fit in Figure \ref{thresh_r_linfit} leads to an underestimation of the preferred frequency. Partially, this can be explained by the fact that the linear regression overestimates the importance of a number of high-threshold-outliers which leads to a shallower slope than what would be appropriate for the "working point" of oscillatory behaviour. Another possible explanation is the fact that "instant spread" is only an approximation. Rather, one could picture the nitrous oxide just to fill a fraction of the available space on a short time scale by replacing $L^2$ by a smaller area. This leads to a stronger local increase of concentration, which eventually leads to faster oscillations. Either way, since these parameters always appear as a fraction $\alpha/L^2$ in \eqref{Pow_Spec_Theta_Final}, both explanations have the same effect onto the shape of the power spectrum.   

!!! HIER MUSS NOCH EINE ANALYSE HIN, WIE SICH DIE OSCI-AMPLITUDE MIT TAU-TH ÄNDERT !!!

\subsection{Properties of Network Activity}

After the elimination of oscillatory activity in the network, we further investigated the features of spiking behaviour, in particular those of the excitatory population, since these were exposed to homeostasis.
...
\caption{A: Distribution of interspike intervals of excitatory neurons for diffusive and non-diffusive homeostasis, measured over $\mathrm{1000\,s}$. For diffusive homeostasis, only neurons with a mean firing rate in a certain range (see legend) were included into statistics to prevent overlapping of different firing rates. Black lines are fits of exponential functions. B: Distribution of coefficients of variation. Each sample represents the CV of one excitatory neuron.}
\label{ISI_CV_compare}
\end{figure}
Figure \ref{ISI_CV_compare} shows the distribution of interspike intervals (ISIs) and the distribution of coefficient of variation ($\mathrm{CV\equiv\sigma/\mu}$), both taken over the set of excitatory neurons. However, in order to provide comparability between the ISI distributions in the diffusive/non-diffusive case, we restricted the statistics of the latter one to neurons whose mean firing rate fall into a window with a width of 1 Hz. Apparently, an approximately exponential distribution of ISIs is preserved for diffusive homeostasis, which is characteristic for poisson processes \cite[p. 27]{Theor_Neur_Dayan}. For perfect poissonian spiking, the CV should give a value of 1. CVs that are smaller, as seen in Figure \ref{ISI_CV_compare}, indicate a tendency towards greater regularity. 
